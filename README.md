...
Here are the following possible actions
	...
	Place Drills (Light, Special, Heavy)
		...
		Until a drill is on a concession that concession does not earn money
		Use the magnifying glass cursor to select the concession where you want to place the drill
...
Phase 3
-------
	Phase 3 is where all the money is worked out
	First the weather for each quadrant is shown in the top right corner
		...
		Gale weather = 0% production and any drills in that quadrant are destroyed (sending them to the bank)
	...
	Next the money card is turned over in the bottom left
	This dictates how much money per barrel is to be earned. it can be $4, $10 or $20

	Next the government card is turned over.
...
Major Variables
	...
	Board
		BO(x=6,y=6) - Board Square Owner - player index who owns this square, -1 = nobody, 0 = Bank (square tested, not owned) 1-6 = Player number
		BW(x=6,y=6) - Board Square Water Depth - 1 = Shallow, 2 = Reef, 3 = Deep
		BD(x=6,y=6) - Board Square Yield - 1 = Shallow, 2 = Reef, 3 = Deep
		BR(x=6,y=6) - Board Square Rig - -1 = No rig present, 1-6 = player number who owns rig
	...
	Players
		...
		PM(PN) = players cash
		PL(PN) = player loan
		PP(PN) = player status, 1=playing,0=Bankrupt,1=Won
	...
	Cards
		...
		Government
			...
			GN = GV(GC,1) = Government ID (1=Conservative,2=Labour,3=Liberal,4=Nationalist)
	...
	Drill/Concession Counts for a player
		CS	= Shallow Concessions on the board
		CR	= Reef Concessions on the board
		CH	= Deep Concessions on the board

		FS	= Shallow Concessions on the board with no drill on it
		FR	= Reef Concessions on the board with no drill on it
		FH	= Deep Concessions on the board with no drill on it

		PS	= Light drills owned
		PR	= Special drills owned
		PH	= heavy drills owned

		IS	= last free Light drill index (0 = None free)
		IR	= last free Special drill index (0 = None free)
		IH	= last free Heavy drill index (0 = None free)

		TS	= Total free Llight drills
		TR	= Total free Special drills
		TH	= Total free Heavy drills
...
Procedures
	...
	INIT ACTION MENU / MAKE ACTION MENU
		This constructs the values of MC$() / AA() and MN so that the menu can be contextual. It cuts out certain possibilities (by setting AA(I) to 0) based on conditions like lack of funds tc
		AA(menu possible) = Action Allowed each index of this array represents whether a menu item is allowed
			A value of = indicates not allowed - anything else represents allowed
		...
		MM() this array represents the "menu meaning" it represents the index into the AA array for a particular menu choice as menus may have missing options
...
Assembly Code
	...
	I know all of this could be written more efficiently and neatly but I was often writing this on the fly with my brother on the other end of the phone waiting for me to finish the code so we could progress the game itself. I am also 40 years out of practice in 6502 assembly :)
